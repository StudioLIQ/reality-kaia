...
contract Reality_Templates_E2E is Reality_Fixture {
    ...
    function test_templates_opening_enforcement() public {
        ...
        assertEq(getBestAnswer(qid), tc.answer1, string.concat("Template ", vm.toString(tc.templateId), ": Answer should be recorded"));
    ...
    function test_templates_timeout_finalization() public {
        for (uint i = 0; i < cases.length; i++) {
            ...
            assertFalse(isFinalized(qid), string.concat("Template ", vm.toString(tc.templateId), ": Should not be finalized before timeout"));
            
            // Test: Can finalize after timeout
            warpTo(uint256(lastAnswerTs) + uint256(timeoutSec));
            reality.finalize(qid);
            
            assertTrue(isFinalized(qid), string.concat("Template ", vm.toString(tc.templateId), ": Should be finalized after timeout"));
    ...
    function test_templates_competition_and_fees() public {
        for (uint i = 0; i < cases.length; i++) {
            ...
            for (uint j = 0; j < 4; j++) {
                ...
                assertEq(getBestAnswer(qid), answers[j], string.concat("Template ", vm.toString(tc.templateId), ": Best answer should update"));
    ...
    function test_templates_no_answer_no_finalization() public {
        ...
        for (uint i = 0; i < cases.length; i++) {
            ...
            assertFalse(isFinalized(qid), string.concat("Template ", vm.toString(tc.templateId), ": Should not finalize without answers"));
    ...
    function test_templates_minimum_bond_doubling() public {
        for (uint i = 0; i < cases.length; i++) {
            ...
            assertEq(getBestBond(qid), initialBond * 2, string.concat("Template ", vm.toString(tc.templateId), ": Bond should double"));
    ...
    function test_templates_fee_calculation_precision() public {
        ...
        for (uint i = 0; i < testBonds.length; i++) {
